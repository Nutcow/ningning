(function initDailyNews() {
        ...
        currentNewsList.forEach(news => {
            ...
            if (news.hasImage) {
                imgHTML = `<div style="width: 150px; height: 100px; background: #f0f0f0; flex-shrink: 0; margin-right: 18px; overflow: hidden; border: 1px solid #f0f0f0;">
                                <img src="${news.imgSrc}" loading="lazy" decoding="async" style="width: 100%; height: 100%; object-fit: cover; transition: 0.3s;" onmouseover="this.style.transform='scale(1.05)'" onmouseout="this.style.transform='scale(1)'">
                           </div>`;
            }
    ...
    window.newsOpenArticle = function(id) {
        ...
        // 有配图的新闻(如"魔法兔兔"的对照表)直接把图片嵌进正文,玩家无需把图片拖到网页外查看
        let imageHtml = '';
        if (news.hasImage && news.imgSrc) {
            imageHtml = `<div style="margin: 0 0 24px; text-align: center;">
                <img class="news-detail-img" src="${news.imgSrc}" alt="配图" style="max-width: 100%; height: auto; border-radius: 4px; display: block; margin: 0 auto;" loading="lazy">
                <div style="font-size: 12px; color: #999; margin-top: 8px;">点击查看大图</div>
            </div>`;
        }
        document.getElementById('news-detail-content').innerHTML = imageHtml + formattedContent;

        // 点击配图用系统"照片查看器"放大查看
        const detailImg = document.querySelector('#news-detail-content .news-detail-img');
        if (detailImg && typeof window.openApp === 'function') {
            detailImg.style.cursor = 'zoom-in';
            detailImg.onclick = () => {
                const titleEl = document.getElementById('title-photoviewer');
                if (titleEl) titleEl.innerText = '照片查看器';
                const viewerImg = document.getElementById('viewer-img');
                if (viewerImg) viewerImg.src = news.imgSrc;
                window.openApp('photoviewer');
            };
        }
